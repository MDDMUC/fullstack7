/**
 * DAB Watermark Component
 * 
 * Figma Reference: node 486:1605
 * Used across all onboarding screens as a subtle background element.
 * 
 * EXACT FIGMA VALUES:
 * - Container: blur-[2px] filter opacity-20 h-[163px] w-[287px]
 * - Default position: left-[51px] top-[92px] (can be customized via props)
 * - Fill: Dark color (#0a0c10) to create silhouette effect at 20% opacity
 * 
 * Position variants from Figma:
 * - Signup screen (482-530): top-[84px]
 * - Step 1 Basic Profile (482-1122): top-[92px]
 */

interface DabWatermarkProps {
  /** Top position in pixels (default: 92px for step1) */
  top?: number;
  /** Left position in pixels (default: 51px) */
  left?: number;
  /** Additional CSS classes */
  className?: string;
}

export default function DabWatermark({ 
  top = 92, 
  left = 51,
  className = '' 
}: DabWatermarkProps) {
